...
규칙 하나를 지웠다가 다음 날 필요해지면, 다시 쓰지 않고 이력에서 되살린다.

```
git log --oneline PROTOCOL.md          # 언제 무엇이 바뀌었는지
git diff HEAD~1 -- PROTOCOL.md         # 직전 변경 내용
git checkout HEAD~1 -- PROTOCOL.md     # 직전 버전으로 되돌리기 (커밋 전 상태로 복원됨)
```

Claude에게는 "어제 지운 규칙 복구해줘"라고 하면 된다.

## 결정 이력

- 2026-08-30 남길 유형 5개 전부 유지 / 자동 발송 대상 아직 없음(전부 초안) / 견적 참조=김영호 black@ / 브리핑=매일 08:00 메일 1회
